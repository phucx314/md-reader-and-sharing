  ...
  const handleSave = async (silent = false): Promise<string | null> => {
    ...
    try {
      ...
      if (!targetUri || initialName !== finalFilename) {
        let proposedUri = `${DIR_URI}${finalFilename}`;
        let counter = 1;
        while (true) {
          if (proposedUri === uri) break; // Renaming to itself
          const info = await FileSystem.getInfoAsync(proposedUri);
          if (!info.exists) break;
          
          finalFilename = `${safeName} (${counter}).md`;
          proposedUri = `${DIR_URI}${finalFilename}`;
          counter++;
        }
        
        if (!targetUri) {
          targetUri = proposedUri;
          currentFileId = generateUUID();
          setFileId(currentFileId);
          setFilename(finalFilename.replace('.md', ''));
        } else {
          try {
            await FileSystem.moveAsync({ from: targetUri, to: proposedUri });
          } catch (err) {
